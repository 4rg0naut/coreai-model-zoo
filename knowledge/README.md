...
- [`ane-vs-gpu-iphone-2026-09.md`](ane-vs-gpu-iphone-2026-09.md) — **Neural Engine vs GPU on the iPhone 17 Pro, one
  ...
  **§9 (2026-09-16): Apple's 6-bit / mixed 4-8 / 4-bit g8 recipes at 2B on the ANE** — 6-bit runs (1.70 GB of weights,
  38.5 tok/s, cold program build 23 min) and now ships as the 2B's `ios-ane-h18p/`; no sub-8-bit palettization is token-exact
  on a 109-token answer; 4-bit g8 will not compile for the ANE; the specialization cache can be pre-seeded per container.
