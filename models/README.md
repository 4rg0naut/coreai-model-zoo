...
| [`minicpm5-1b.md`](minicpm5-1b/README.md) | MiniCPM5-1B (plain Llama dense, OpenBMB) | [🤗 MiniCPM5-1B-CoreAI](https://huggingface.co/mlboydaisuke/MiniCPM5-1B-CoreAI) | 1.08B int8 per-block-32 (rebuilt 2026-09-09; the per-channel bundle had dead LM-head rows and never halted); plus `ios-ane-h18p/` — stock static iOS export, **8-bit** palettized (1.3 GB), **Neural Engine**, device gate PASS 3/3 (4-bit fails), **69.6 / 58.6** tok/s decode (llm-benchmark protocol); 24/24 + stop-exact vs HF fp32 on device, **61.7 tok/s iPhone 17 Pro**, 246.6 tok/s M4 Max (pipelined, llama→mistral remap) |
| [`minicpm5-2b`](minicpm5-2b/README.md) | MiniCPM5-2B (plain Llama dense, 42L, OpenBMB — the 1B recipe one YAML apart: int8 per-block-32) | [🤗 MiniCPM5-2B-CoreAI](https://huggingface.co/mlboydaisuke/MiniCPM5-2B-CoreAI) | 2.5B int8 per-block-32 (2.7 GB), **16/16 token-exact vs HF fp32** (margin-aware gate), **22.4 tok/s iPhone 17 Pro** / 127.6 M4 Max (pipelined, llama→mistral remap); plus `ios-ane-h18p/` — stock static iOS export, 4-bit palettized (1.4 GB), **Neural Engine**, device gate PASS 3/3, 48.0 / 38.2 tok/s decode (llm-benchmark protocol) |
