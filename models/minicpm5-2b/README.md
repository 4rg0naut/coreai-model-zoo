...
- **Neural Engine bundle (`ios-ane-h18p/`, 2026-09-15)** — Apple's stock static iOS export: `coreai.llm.export openbmb/MiniCPM5-2B --platform iOS --compression 4bit_weight_palettized_group32 --max-context-length 4096` (k-means 4-bit, group 32; embeddings int8; static graphs `prompt_opt`/`extend` × {256, 512, 1024, 2048, 4096} × {8, 16, 64}), then `xcrun coreai-build compile … --platform iOS --preferred-compute neural-engine --architecture h18p` → 31/31 ANE regions. Same `llama → mistral` remap, otherwise Apple's iOS Mistral builder untouched. Loads through the unmodified Apple main `StaticShapeEngine` (`EngineFactory` auto-detects the chunked-static structure). `ios-static/` is the same export before AOT — the portable IR, compile it yourself for another chip. Gated on the phone with a teacher-forced single-step sweep plus a free-run rollout against the fp32 oracle (`--ios-ane` in `export_minicpm5.py`; tool and rules in [`../../knowledge/minicpm5-1b.md`](../../knowledge/minicpm5-1b.md) §2026-09-15). The 1B sibling **fails** this gate at 4-bit (two margin-clear flips on a chat turn, unchanged with fp16 embeddings) and ships at **8-bit** palettization instead. **2026-09-16 fidelity probe on a longer chat prompt** ("Explain in a short paragraph why the sky is blue.", no-think, greedy, identical prompt ids on both bundles, fp32 `transformers` oracle 109 tokens): the **4-bit ANE bundle diverges from fp32 at step 0** (oracle `The` at margin 1.0, bundle `To`) and its paragraph is not the fp32 answer (and gets the physics wrong); the **int8 GPU bundle matches fp32 for 19 tokens** and diverges only on a 0.094 near-tie below the floor — [`fidelity-probe-sky-2026-09-16.json`](fidelity-probe-sky-2026-09-16.json). The 3-prompt gate (48 steps) did not catch this; treat the 4-bit ANE bundle as **fast but not fp32-faithful on free-form answers** (the 8-bit 2B loads but never returns its first generation on the ANE). **2026-09-16 — Apple's other iOS recipes at 2B, gated with the sky prompt added to the fixture (4 prompts, 157 teacher-forced steps):** none reaches fp32 to the floor, so **`ios-ane-h18p/` stays the 4-bit bundle with the caveat above**. Apple's registry 6-bit preset shape (k-means 6-bit g8, [`conversion/minicpm5_pal6_g8.yaml`](../../conversion/minicpm5_pal6_g8.yaml), 2.5 GB, resources.bin 1.70 GB) **does run on the ANE** — natural 24/24, chat 8/8 incl. the stop, long 16/16, sky 33 tokens exact then a flip on a 0.168-margin step (5 margin-clear flips / 109; the same first flip in an fp32 simulation of the recipe, so it is the recipe, not the chip) — [`gate-minicpm5-2b-ane-6bit-g8-FAIL.json`](gate-minicpm5-2b-ane-6bit-g8-FAIL.json); cold ANE program build **1406 s**, warm 0.2 s, footprint 2.6 GB, decode **38.5 tok/s** thermal-recovered (the GPU int8 bundle 23.8 in the same run, the 4-bit ANE bundle ~55) — [`bench-iphone-ane-6bit-vs-gpu-2026-09-16.json`](bench-iphone-ane-6bit-vs-gpu-2026-09-16.json). Apple's mixed 4/8 shape (4-bit g32 + the nine most 4-bit-sensitive layers at 8-bit, [`conversion/minicpm5_mixed_4bit_8bit_g32.yaml`](../../conversion/minicpm5_mixed_4bit_8bit_g32.yaml), 1.6 GB) runs but flips the chat turn at step 5 and the sky answer at step 9 — [`gate-minicpm5-2b-ane-mixed48g32-FAIL.json`](gate-minicpm5-2b-ane-mixed48g32-FAIL.json). The 4-bit g8 preset (and the mixed shape on a g8 base) cannot be compiled for the ANE at all: `coreai-build` reports `ANECCompileOffline … CompilationFailure` on the first graph and silently emits 0 ANE regions. Boundary on this phone: 1.70 GB of weights runs, 2.16 GB (8-bit) loads but never returns. Details and the fp32 recipe simulation: [`../../knowledge/ane-vs-gpu-iphone-2026-09.md`](../../knowledge/ane-vs-gpu-iphone-2026-09.md) §9.
